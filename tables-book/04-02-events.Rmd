...
heading <- tabular(Heading("")*1*
                     Heading("")*count ~ 
                   Heading()*ARM*(
                     Heading("n (% of subjects)")*1 +
                     Heading("Total events")*1), data = adsl)

body <- tabular( Heading("Patients with at least one event")*1*
                   Heading("")*Heading("")*USUBJID +
                 Heading()*USUBJID*Heading()*AEBODSYS*
                   (Heading("Patients with at least one event")*1 +
                    Heading()*AEDECOD*DropEmpty()) ~
                 Heading()*ARM*(
                   Heading("n (% of subjects)")*Percent(denom = ARM, fn = countpercentid) +
                   Heading("Total events")*1), 
                 data = ex_adae )
